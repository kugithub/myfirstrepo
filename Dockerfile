FROM docker.apple.com/wwrc/pos-buildozer:j3.0 as builder
WORKDIR /code
ADD . /code
USER root
RUN source /tmp/.bash_profile && \
    ...
    openssl enc -aes-256-cbc -salt -in /code/service/target/wwrc-customer-service-18.8.4-jar-with-dependencies.jar -out /code/service/target/wwrc-customer-service-18.8.4-jar-with-dependencies.jar.enc -pass file:./artifactkey.bin

FROM docker.apple.com/wwrc-docker-releases/jamaica-alpine-jre8:jaj1.5
ENV LANG C.UTF-8

WORKDIR /
COPY --from=builder /code/service/target/wwrc-customer-service-18.8.4-jar-with-dependencies.jar.enc wwrc-customer-service-18.8.4-jar-with-dependencies.jar.enc
...
RUN touch startup.sh && echo "#!/bin/sh" >> startup.sh && \
    ...
    echo "openssl enc -d -aes-256-cbc -in wwrc-customer-service-18.8.4-jar-with-dependencies.jar.enc -out wwrc-customer-service-18.8.4-jar-with-dependencies.jar -pass file:./artifactkey.bin" >> startup.sh && \
    ...
    echo "java -classpath pos-encrypt-util-1.1.4.jar:wwrc-customer-service-18.8.4-jar-with-dependencies.jar \\" >> startup.sh && \
