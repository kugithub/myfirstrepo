...
RUN source /tmp/.bash_profile && \
    ...
    openssl enc -aes-256-cbc -salt -in /code/service/target/wwrc-configuration-server-service-19.1.3-jar-with-dependencies.jar -out /code/service/target/wwrc-configuration-server-service-19.1.3-jar-with-dependencies.jar.enc -pass file:./artifactkey.bin
...
WORKDIR /
COPY --from=builder /code/service/target/wwrc-configuration-server-service-19.1.3-jar-with-dependencies.jar.enc wwrc-configuration-server-service-19.1.3-jar-with-dependencies.jar.enc
...
RUN touch startup.sh && echo "#!/bin/sh" >> startup.sh && \
    ...
    echo "openssl enc -d -aes-256-cbc -in wwrc-configuration-server-service-19.1.3-jar-with-dependencies.jar.enc -out wwrc-configuration-server-service-19.1.3-jar-with-dependencies.jar -pass file:./artifactkey.bin" >> startup.sh && \
    ...
    echo "java -classpath pos-encrypt-util-1.1.4.jar:wwrc-configuration-server-service-19.1.3-jar-with-dependencies.jar \\" >> startup.sh && \
    ...
    echo "-Did_groups=NO_PULL \\" >> startup.sh && \
